...
//next, to get a sine wave set up
SinOsc s => ADSR e => dac;
e.set(15::ms, 150::ms, 0, 50::ms);
while (true) {
	int note;
	...
	Math.mtof(note) => s.freq; //use mtof to convert it from midi numbering to a frequency so it can be played
	e.keyOn();
	200::ms => now;
}
